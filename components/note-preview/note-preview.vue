<template>
	<view class="note-preview-wrap" style="padding: 10px;">
		<!-- 预览内容滚动区域 -->
		<!-- <scroll-view scroll-y class="preview-content"> -->
		<!-- 内容块列表 -->
		<view class="preview-block" v-for="(item, index) in contentList" :key="item.id"
			...
			<view v-if="item.dataType === 'image'" class="preview-media-wrap">
				<image :src="item.content" mode="widthFix" class="preview-img" @error="handleImageError" />
			</view>

			<!-- 视频块预览 -->
			<view v-if="item.dataType === 'video'" class="preview-media-wrap">
				<video :src="item.content" class="preview-video" controls show-center-play-btn />
			</view>
			<!-- 个人名片 -->
			<uni-card 
			v-if="item.dataType === 'user'"
			     @click="handleClickUserCard(item)" margin="0">
				...
				<view class="user">
					...
					<uni-icons type="forward" color="#999" size="30" style="flex:1;text-align: right;"></uni-icons>
