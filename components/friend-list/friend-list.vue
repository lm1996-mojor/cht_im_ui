			...
			>
				<view class="note-card">
					...
					<view class="card-header">
						<user-avatar class="avatar" :model-value="item.friendInfo.avatar" />
						<view class="header-info">
							<view class="title-box">
								<text class="note-title">{{ item.friendInfo.nickname || '未命名好友' }}</text>
							</view>
						</view>
					</view>

					<view class="note-content" v-html="item.description || '暂无好友介绍'"></view>
				</view>
			</view>
			<view class="empty-tip" v-if="filteredContent.length === 0 && !isLoading">
				{{ searchKeyword ? '未找到匹配的好友' : '暂无好友数据' }}
			</view>
		</view>
