  ...
  <view class="bgColor">
    <view class="wxgrouplist">
      <view
        ...
        v-for="(v, i) in list"
        :key="i"
        @click.stop="handleSelectItem(v)"
      >
        <!-- 修复：1. 传入正确的循环项 v  2. 绑定 v.id 匹配选中状态 -->
        <view class="radio-btn">
          <view
            class="radio-icon"
            :class="selectedCircleId === v.id ? 'checked' : ''"
          ></view>
        </view>
...
export default {
  data() {
    return {
      list: [],
      selectedCircleId: '' // 修复：2. 定义核心选中变量，用于匹配单选样式
    }
  },
  computed: {},
  onLoad(e) {},
  ...
  methods: {
    ...
    // 修复：3. 实现 handleSelectItem 方法，更新选中状态变量
    handleSelectItem(item) {
      // 赋值选中项的ID，用于匹配单选样式的 checked 类
