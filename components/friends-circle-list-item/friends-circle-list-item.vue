<template>
	<view class="friendsCircle-content-item" v-for="(v, i) in content" :key="i">
		<image class="friendsCircle-content-avatar" :src="v.authorInfo.avatar" mode="aspectFill" @click="gofriend(v)">
		</image>
		<view class="friendsCircle-content-right">
			<view class="friendsCircle-content-nickName" @click="gofriend(v)">{{ v.authorInfo.nickname }}</view>
			<view class="friendsCircle-content-text">{{ v.content }}</view>
			<view class="friendsCircle-content-imgs" v-if="v.files.length>0">
				<image v-for="(item, ii) in v.files" :key="ii" class="friendsCircle-content-img"
					:class="'fci' + v.files.length" :src="item.url" mode="aspectFill" @click="pimgs(v.files,ii)">
				</image>
			</view>
			<view class="friendsCircle-content-video" @click="openVideo(v.files[0].videoUrl)"
				v-if="v.topicType=='VIDEO'&&v.files.length>0">
			...
			<view class="friendsCircle-content-location" v-if="v.location" @click="goMap(v.location)">{{ v.location }}
			</view>
			...
			<view class="friendsCircle-like" v-if="v.interactions&&v.interactions.length>0">
				<view class="friendsCircle-like-icon wxfont zan"></view>
				<text class="friendsCircle-like-text" v-for="(z,zindex) in v.interactions.filter((item)=>item.type==1)">{{z.userInfo.nickname}}</text>
			</view>
			<view class="friendsCircle-comment" v-if="v.interactions&&v.interactions.length>0">
				<view class="friendsCircle-comment-item" v-for="(c,ci) in v.interactions.filter((item)=>item.type==2)" :key="ci">
					<text class="friendsCircle-comment-text" @click="gofriend(c)">{{c.userInfo.nickname}}</text>
					<view class="friendsCircle-comment-feedback" v-if="c.replyToInfo?.id">
						回复
						<text class="friendsCircle-comment-text" @click="gofriend(c)">{{c?.replyToInfo?.nickname}}</text>
					</view>
					<view class="friendsCircle-comment-content" v-if="c.commentContent" @click="replyclick(c,ci,i)">
			...
			<view class="zfb-tk-send-tool-c">
				<view class="zfb-tk-send-tool-input-box" @click="msgFocus=true">
					<textarea class="zfb-tk-send-tool-input" :adjust-position="false" :focus="msgFocus" v-model="msg"
						:placeholder="placeholder" confirm-type="send" @confirm="sendMsg(msg)" hold-keyboard
						:maxlength="-1" auto-height />
				</view>
				<view class="zfb-tk-send-tool-text" @click="sendMsg(msg)"
		...
		methods: {
			replyclick(e, ci, i) {
				...
				if (e.canDeleted == 'Y') {
					...
					this.replydata = e
					this.type = 'reply'
					this.placeholder = '回复' + e.userInfo.nickname
					this.$nextTick(() => {
			...
			openVideo(e) {
				this.$fc.plusDownload({
					onlinePath: e
				}).then(res => {
					this.$fc.plusOpenFile({
						filePath: res
					})
				})
			},
			...
			sendMsg(e) {
				...
				const data=this.content[this.chooseUserIndex]
				this.$http.request({
					url: '/chat_im/moments/comment',
					method: 'POST',
					data: JSON.stringify({
						momentId: data.id,
						parentId: this?.replydata?.userInfo?.id,
						content: e
					}),
					success: (res) => {
						if (res.data.code == 200) {
