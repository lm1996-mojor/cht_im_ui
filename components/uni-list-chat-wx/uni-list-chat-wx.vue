<template>
	...
	<cell>
	<!-- #endif -->
		<view :hover-class="!clickable && !link ? '' : 'uni-list-chat--hover'" :style="{background:color}"
			class="uni-list-chat" @longpress="longpressItem($event,itemKey,item)">
			...
			<view class="uni-list-chat__container" @click="onClick">
				<view class="uni-list-chat__header-warp">
					<view v-if="item.conversationsType==1" class="uni-list-chat__header"
						:class="{ 'header--circle': avatarCircle }">
						<image class="uni-list-chat__header-image" 
						 :src="item.avatar>0?`../../static/img/avatar/avatar${item.avatar}.jpg`:item.avatar"
						 mode="aspectFill"></image>
					</view>
					<!-- 头像组 -->
					<view v-if="item.conversationsType=='2'" class="uni-list-chat__header">
						<view v-for="(item, index) in JSON.parse(item.avatar||'[]')" :key="index" class="uni-list-chat__header-box"
				...
				<view class="uni-list-chat__content">
					...
					<view class="uni-list-chat__content-extra">
						<slot>
							<text class="uni-list-chat__content-extra-text">{{ formatTime(item?.latestMessage?.createdAt) }}</text>
							...
							<view v-if="badgeText && badgePositon === 'right'" class="uni-list-chat__badge"
								...
								<text
									class="uni-list-chat__badge-text">{{ badgeText === 'dot' ? '' : badgeText }}</text>
							</view>
						</slot>
					</view>
				</view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			avatarList() {
				return this.returnAvatar(this.item.avatar)
			},
			computedAvatar() {
