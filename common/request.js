...
let http = {
	...
	'beforeRequestFilter': (config) => {
		console.log('beforeRequestFilter', config)
		return config
	},
...
function request(con) {
	...
	let config = { //判断是否有以下属性没有就赋予默认值
		...
		complete: con.complete ? (res) => {
			...
			if (res.statusCode == 401) { //登录过期
				uni.showToast({
					title: '登录已过期，请重新登录!',
					icon: 'none'
				});
				this?.$ws?.clearSocketTask() // 关闭连接
				uni.hideLoading();
				uni.clearStorage()
				setTimeout(() => {
...
function post(url, data, con, success) {
	console.log('rul_post', url, data, con)
	let conf = {};
	if (con && typeof con == 'function') {
		if (success && typeof success == 'object') {
			conf = success
...
function uploadFile(con) {
	...
	for (var i = 0; i < AUTH_TOKEN.length; i++) {
		if (uni.getStorageSync(AUTH_TOKEN[i])) {
			config.header[AUTH_TOKEN[i]] = uni.getStorageSync(AUTH_TOKEN[i]);
		}
	}
...
// ======================== 核心：分环境动态配置baseUrl ========================
// #ifdef H5
// H5环境：清空baseUrl，让请求走【相对路径】，被Vite代理拦截，解决跨域！！！
http.setBaseUrl("");
// #endif

// #ifndef H5
// 非H5环境(APP/小程序)：保留绝对路径，原生请求无跨域，正常访问！！！
http.setBaseUrl("http://124.220.24.143"); //在线服务器
// http.setBaseUrl("http://chat.blinx.asia/"); //在线服务器
