...
export default {
	...
	previewImagesolo(File) {
		if (File > 0) {
			uni.previewImage({
				urls: [`../static/img/avatar/avatar1.jpg`],
			});
		} else {
			uni.previewImage({
				urls: [File],
	...
	loadMore({
		...
		const queryStr = new URLSearchParams({
			...
			pageSize: 4 //queryParams.pageSize
		}).toString();
		return new Promise((resolve, reject) => {
			http.request({
				url: url + queryStr,
				success: res => {
					if (res.data.code == 200) {
						...
						let pageable = res.data.data.pageable;
						if (pageable.totalPages === queryParams.page) {
							// queryParams.status = 'noMore'
						}
						// console.log('encodeURIComponent(JSON.stringify(obj))', url +
						// 	encodeURIComponent(JSON.stringify(queryParams)))
						const content = res.data.data.content;
						if (queryParams.refreshing && res.data.rows == '' || queryParams
							.refreshing && res.data.data.content.length == 0) {
							queryParams.status = 'noMore'
							resolve({
	...
	pushOutMsg({
		...
		console.log('this.$fc', this);
		// http.request({
		// 	url: '/chat_im/message/send',
		// 	method: 'POST',
		// 	data: JSON.stringify(msgOnlie),
		// 	success: res => {
		// 		console.log('ssssocke',res);
		// 	},
		// 	fail: res => {
		// 		msgSendType = 'error';
		// 	}
