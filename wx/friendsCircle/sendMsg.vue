  ...
  <view class="friendsCircle-send">
    <form @submit="subform">
      <view class="friendsCircle-send-item">
        <textarea
          class="friendsCircle-send-textarea"
          auto-height
          placeholder="这一刻的想法..."
          maxlength="200"
      ...
      <view class="friendsCircle-send-item">
        <view class="friendsCircle-send-itemxlist">
          <view
            class="friendsCircle-send-itemx"
            :class="{ lcation: location }"
            @click="getlocation"
          >
            <view class="friendsCircle-send-itemx-icon wxfont dingwei2"></view>
            <view class="friendsCircle-send-itemx-c">
              <text class="friendsCircle-send-itemx-text" v-if="location">{{
                location.name
              }}</text>
...
export default {
  ...
  methods: {
    ...
    subform(e) {
      ...
      if (checkRes) {
        this.$http.request({
          url: '/chat_im/moments/add',
          method: 'POST',
          data: JSON.stringify({
            ...this.form,
            mediaUrls: this.form.files.map(item => item.url)
          }),
          success: res => {
...
.lcation .friendsCircle-send-itemx-icon,
.lcation .friendsCircle-send-itemx-text {
  color: #05c160;
}
</style>
