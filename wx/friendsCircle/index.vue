<template>
  <view class="friendsCircle">
    ...
    <view class="friendsCircle-content">
      <view
        class="friendsCircle-msgalert-box"
        v-if="topicReply.count && topicReply.count > 0"
        @click="gomsglist"
      >
        <view class="friendsCircle-msgalert">
          <image
            class="friendsCircle-msgalert-img"
            :src="topicReply.portrait"
            mode="aspectFill"
          ></image>
          <text class="friendsCircle-msgalert-text"
            >{{ topicReply.count }}条新消息</text
          >
        </view>
      </view>
      <friends-circle-list-item
        ref="fcli"
...
export default {
  ...
  methods: {
    ...
    getlist() {
      ...
      this.$fc
        .loadMore({
          url: '/chat_im/moments/page?',
          queryParams: this.queryParams
        })
        .then(res => {
          this.content = (res.list || []).map(item => {
            const type = typeof item.mediaUrls
            const urls =
              item.mediaUrls && item.mediaUrls != 'null'
                ? JSON.parse(item.mediaUrls || '[]').map(url => ({
                    url
                  }))
                : []
            return {
              ...item,
              interactions: item?.interactions?.reverse?.(),
              files: urls
            }
          })
