<template>
	<view class="friendsCircle">
		...
		<view class="friendsCircle-content">
			...
			<friends-circle-list-item ref="fcli" v-model:content="content" @refresh="handleRefresh"></friends-circle-list-item>
		</view>
		...
		getlist() {
			...
			this.$fc.loadMore({
				...
				this.content=(res.list ||[]).map((item)=>{
					...
					const urls=item.mediaUrls && item.mediaUrls != 'null'  ?JSON.parse(item.mediaUrls||'[]').map((url)=>({
							url
						})):[]
					return {
						...item,
