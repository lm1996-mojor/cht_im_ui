		...
		<uni-list-wx>
			<uni-list-wx :border="true">
				...
				<template v-for="(v, i) in toplist" :key="i">
					<uni-list-chat-wx color="#eeeeee" :itemKey="i" :item="v" @longpressItem="longpressItem" :longTapItemKey="longTapItemKey" @click="clickChat(v)" clickable :title="v.nickName" :note="v.content" :time="v.time" badge-positon="left" :badge-text="v.unreadCount"></uni-list-chat-wx>
				</template>
				<!-- 非置顶列表 -->
				<template v-for="(v, i) in NOTtoplist" :key="i">
					<uni-list-chat-wx color="#fff" :itemKey="i" :item="v" @longpressItem="longpressItem" :longTapItemKey="longTapItemKey" @click="clickChat(v)" clickable :title="v.nickName" :note="v.content" :time="v.time" badge-positon="left" :badge-text="v.unreadCount"></uni-list-chat-wx>
				</template>
			</uni-list-wx>
		</uni-list-wx>
	...
	onLoad() {
		this.clickToSubmitSure = this.$fc.debounce(
			e => {
				...
				// })
				uni.navigateTo({
					url: '../chatWindow/index?userId=' + toUserId+'&conversationsId='+e.id+'&conversationsType='+e.conversationsType+'&groupId='+e.groupId
				})
			},
			1000,
			true
