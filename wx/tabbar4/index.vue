<template>
  <view class="bgColor">
    ...
    <view class="menu-list">
      ...
      >
        <view class="menu-icon" :class="getIconBgClass(index)">
          <i class="fas" :class="getIconClass(index)"></i>
        </view>
    ...
    <view class="menu-list">
      <view class="menu-item" id="settings-item" @click="handleLogout">
...
const $fc = instance.appContext.config.globalProperties.$fc
  ...
  $http.request({
    ...
    success: res => {
      if (res.data.code == 200) {
        data.value = res?.data?.data?.detail||{}
      }
    },
    fail: () => {
      uni.showToast({
...
const getIconBgClass = (index) => {
  const bgClasses = ['bg-green', 'bg-teal', 'bg-orange', 'bg-purple']
  return bgClasses[index] || 'bg-green'
}

// 根据索引获取图标类名
const getIconClass = (index) => {
  const iconClasses = [
    'fa-clipboard-list', // 我的笔记
    'fa-qrcode',         // 扫一扫
    'fa-users',          // 我的圈子
    'fa-comment-dots'    // 我的朋友圈
  ]
  return iconClasses[index] || 'fa-clipboard-list'
}
...
// 挂载导航栏按钮点击事件到实例
instance.proxy.onNavigationBarButtonTap = onNavigationBarButtonTap
</script>
