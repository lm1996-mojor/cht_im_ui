      ...
      >
        <view class="note-card">
          <view class="card-header">
            <user-avatar
              class="avatar"
              :modelValue="item.userInfo?.avatar"
              mode="aspectFill"
            />
            <view class="header-info">
              ...
              <view class="member-info">
                <text
                  class="member-count"
                  @click.stop="toCircleDetail(item.circleId)"
          ...
          <view class="img-box" v-if="item.files && item.files.length > 0">
            <image
              class="img-item"
              v-for="(imgUrl, imgIndex) in item.files"
              :key="imgIndex"
              :src="imgUrl"
              mode="aspectFill"
              @click.stop="previewImg(item.files, imgIndex)"
            />
          </view>
        </view>
      </view>
...
export default {
  ...
  methods: {
    ...
    previewImg(imgArr, currentIndex) {
      if (!imgArr || imgArr.length === 0 || currentIndex >= imgArr.length)
        return
      uni.previewImage({
        urls: imgArr,
