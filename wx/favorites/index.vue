    ...
    <view class="favorites-wx">
      ...
      <uni-swipe-action>
        <uni-swipe-action-item
          class="favorites-uni-swipe-action-item"
          v-for="(v, i) in list"
          :key="i"
        >
          <view class="favorites-wx-item">
            <view
              class="favorites-wx-content favorites-wx-content-text"
              v-if="v.collectType == 'TEXT'"
            >
            ...
            <view
              class="favorites-wx-content favorites-wx-content-img"
              v-if="v.collectType == 'IMAGE'"
            >
              <image
                :src="returnParse(v.content).url"
                @click="$fc.previewImagesolo(returnParse(v.content).url)"
                mode="aspectFill"
              ></image>
            </view>
            <view
              class="favorites-wx-content favorites-wx-content-video"
              v-if="v.collectType == 'VIDEO'"
            >
              <view
                class="favorites-wx-content-video-img"
                @click="openVideo(returnParse(v.content).url)"
              >
                <image
                  :src="returnParse(v.content).img"
                  mode="aspectFill"
                ></image>
            ...
            <view
              class="favorites-wx-content favorites-wx-content-location"
              v-if="v.collectType == 'LOCATION'"
            >
              <view
                class="favorites-wx-content-location-box"
                @click="
                  goMap({
                    latitude: returnParse(v.content).latitude,
                    longitude: returnParse(v.content).longitude
                  })
                "
              >
                <view class="favorites-wx-content-location-box-name">{{
                  returnParse(v.content).name
                }}</view>
                <view class="favorites-wx-content-location-box-address">{{
                  returnParse(v.content).address
                }}</view>
                <image
                  class="favorites-wx-content-location-box-map"
                  src="../../static/wx/map.png"
                  mode="aspectFit"
                ></image>
              </view>
            </view>
            <view
              class="favorites-wx-content favorites-wx-content-voice"
              v-if="v.collectType == 'VOICE'"
            >
              <view class="favorites-wx-content-voice-box">
                <view
                  class="favorites-wx-content-voice-content"
                  @click="playVOICE(returnParse(v.content).url)"
                >
            ...
            <view class="favorites-wx-content" v-if="v.collectType == 'CARD'">
              <view
                class="favorites-wx-content-card"
                @click="goAddfriend(returnParse(v.content))"
              >
                <view class="favorites-wx-content-card-top">
                  <image
                    class="favorites-wx-content-card-top-img"
                    :src="returnParse(v.content).avatar"
                  ></image>
                  <view class="favorites-wx-content-card-top-content">
                    <view class="favorites-wx-content-card-title">{{
                      returnParse(v.content).name
                    }}</view>
                    <view class="favorites-wx-content-card-no">{{
                      returnParse(v.content).chatNo
                    }}</view>
                  </view>
                </view>
          ...
          <template v-slot:right>
            <view class="favorites-wx-action-item">
              <view
                class="favorites-wx-action-item-btn favorites-wx-delete"
                @click="favoritesDeleta(v, i)"
                >删除</view
              >
              <view
                v-if="type == '2'"
                class="favorites-wx-action-item-btn favorites-wx-view"
                @click="clickitem(v, i)"
                >发送</view
              >
            </view>
...
.favorites-wx-content-video-img,
.favorites-wx-content-video-img image {
  width: 160rpx;
  height: 160rpx;
  position: relative;
}
