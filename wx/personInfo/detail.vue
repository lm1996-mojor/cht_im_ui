	...
	<view class="bgColor">
		...
		<view class="person-wx" v-if="detail">
			<view class="person-wx-user">
				<image class="person-wx-avatar" 
					@click="$fc.previewImagesolo(detail.portrait)"
					:src="detail?.friendInfo?.avatar>0?`../../static/img/avatar/avatar${detail?.friendInfo?.avatar}.jpg`:detail?.friendInfo?.avatar"
					mode="aspectFill"></image>
				<view class="person-wx-user-detail">
					<view class="person-wx-nikename">
						<text class="text">{{detail?.friendInfo?.nickname}}</text>
						<view class="wxfont person-wx-nikename-icon nan" v-if="detail.gender=='1'"></view>
						<view class="wxfont person-wx-nikename-icon nv" v-if="detail.gender=='0'"></view>
					</view>
					<text class="person-wx-name">微聊号：{{detail?.friendInfo?.account}}</text>
		...
		<tool-list-wx :list="list3" type="btns" @itemClick="itemClick"></tool-list-wx>
	...
	export default {
		...
		methods: {
			...
			setSourceText() {
				...
				if (this.detail.sourceLabel) {
					...
					const sourceMap = {
						1: '扫一扫',2: '名片',3: '微聊号',4: '手机号',
						5: '摇一摇',6: '系统',7: '群聊',8: '附近的人'
					};
			...
			itemClick2(e, i) {
				if (i === 1) {
					uni.navigateTo({ url: `../../wx/friendsCircle/person?userId=${this.userId}` });
				}
			},
			// 添加到通讯录 点击事件
			itemClick4(e, i) {
				if (i === 0) {
					uni.navigateTo({ url: `../search-friends/add?userId=${this.detail.userId}&source=${this.source}` });
				}
			},
			// 发消息/音视频通话 点击事件
			itemClick(e, i) {
					...
					case 0:
						this.$http.request({
							...
							success: (res) => {
								...
								uni.navigateTo({ url: `../chatWindow/index?userId=${this.userId}&conversationsId=${conversationsId}` });
							}
						})
						break;
					// 音视频通话-选择弹窗
					...
					success: (res) => {
						...
						if (res.data.code == '200') {
							if (res.data.data.status !== '0') {
								return uni.showToast({ title: res.data.data.statusLabel, icon: 'none' });
							}
							const uInfo = res.data.data.userInfo;
							uni.setStorage({
								key: 'call',
								data: JSON.stringify({
									userId: uInfo.userId,trtcId: uInfo.trtcId,
									nickName: uInfo.nickName,portrait: uInfo.portrait,
									startTime: new Date().getTime(),type: callType ==1 ? 'audio':'video'
								}),
								success: () => {
		...
		onNavigationBarButtonTap(e) {
			if (this.showtitleNViewBtns && e.index === 0) {
				uni.navigateTo({ url: `edit?data=${encodeURIComponent(JSON.stringify(this.detail))}` });
			}
		}
	}
</script>
...
<style scoped>
	...
	.person-wx-nikename-icon.nan { color: #007AFF; }
	.person-wx-nikename-icon.nv { color: #FF5A5F; }
	.person-wx-nikename-icon { margin-left: 10rpx; }
</style>