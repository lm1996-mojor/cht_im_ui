	...
	<view class="bgColor">
		...
		<view class="person-wx" v-if="detail">
			<view class="person-wx-user">
				<image class="person-wx-avatar" @click="$fc.previewImagesolo(detail.portrait)"
				...
				<view class="person-wx-user-detail">
					<view class="person-wx-nikename">
						...
						<view class="wxfont person-wx-nikename-icon" :class="{'nv':detail.gender=='0'}"
							v-if="detail.gender=='0'"></view>
						<view class="wxfont person-wx-nikename-icon" :class="{'nan':detail.gender=='1'}"
							v-if="detail.gender=='1'"></view>
					</view>
					<text class="person-wx-name">
						微聊号：{{detail?.friendInfo?.account}}
					</text>
...
<script>
	...
	export default {
		...
		methods: {
			getUserInfo(e) { //好友详情
				this.$http.request({
					url: '/chat_im/friend/detail?friendId=' + e,
					success: (res) => {
