<template>
	<view class="bgColor">
		...
		<view class="search-friends">
			...
			<!-- ✅ 新增：搜索结果展示区 - 搜索到数据就显示 -->
			<view class="search-friends-list-title" v-if="searchList.length>0">搜索结果</view>
			<view class="search-friends-list" v-if="searchList.length>0">
				<view class="search-friends-list-item" v-for="(v,i) in searchList" :key="i"  @click="goSearchDetail(v)">
					<!-- <image class="search-friends-list-img" :src="v.avatar" mode="aspectFill"></image> -->
					<user-avatar class="search-friends-list-img" :modelValue="v.avatar" mode="aspectFill"></user-avatar>
					<view class="search-friends-list-user">
						<view class="search-friends-list-username">{{v.nickname}}</view>
						<view class="search-friends-list-remark">{{v.reason || '可添加为好友'}}</view>
					</view>
				</view>
			</view>
			<!-- ✅ 新增：无搜索结果提示 -->
			<view class="no-search-data" v-if="isSearch && searchList.length == 0">未搜索到相关用户</view>

			<!-- 原有：好友申请历史列表 -->
			<view class="search-friends-list-title" v-if="list.length>0">历史申请</view>
			<view class="search-friends-list">
				<view class="search-friends-list-item" v-for="(v,i) in list" :key="i">
					<image class="search-friends-list-img" :src="v.avatar" mode="aspectFill" @click="godetail(v)"></image>
...
<script>
	export default {
		...
		methods: {
			...
			goSearchDetail(e){
				uni.navigateTo({
					url:'../add-search-friends/detail?userId='+e.userId+'&type=1'+'&nickname='+e.nickname
				})
			},
			agreeApply(e){
				this.$http.request({
