		...
		<view class="xw-login-form">
			<form @submit="sublogin">
				...
				<view class="xw-login-form-item">
					...
					<view class="login-form-icon login-form-seepass" @click="form.account=''" v-if="form.account">
						<uni-icons type="clear" size="26" color="#B9CCE0"></uni-icons>
					</view>
				</view>
		...
		methods: {
			...
			sublogin(e) {
				...
				if (checkRes) {
					...
					if (!this.logintype) {
						...
						this.$http.request({
							url: '/chat_im/sso/login',
							method: 'POST',
							data: JSON.stringify(formData),
							success: (res) => {
								console.log('lo--', res)
								if (res?.data?.code == 200) {
									console.log('loginyew---', res.data.data.accessToken)
									this.loginDone(`Bearer ${res.data.data.accessToken}`, res.data.data
										.userInfo)
								} else {
									uni.showToast({
										title: res?.data?.msg || '登录错误',
										icon: 'none'
									});
									return;
								}
			...
			loginDone(token, userInfo) {
				...
				this.$store.dispatch('get_UserInfo').then(res => {
					...
					// #endif
					console.log('connectSocket', this);
