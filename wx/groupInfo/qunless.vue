...
export default {
	...
	methods: {
		createGroup() {
			...
			this.$http.request({
				url: `/chat_im/group/remove/member`,
				method: 'DELETE',
		...
		bindClick(e,select) {debugger
			this.select = select;
		},
		getflist() {
			this.$http.request({
				url: '/chat_im/group/members/page?pageSize=10&pageNumber=1&groupId='+this.groupId,
				method:'GET',
				success: res => {
					if (res.data.code == 200) {
						this.list=(res.data.data.content||[]).map((item)=>({
							...item.userInfo
						}))
