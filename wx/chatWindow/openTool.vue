<template>
  <view
    class="openTool"
    :class="itemKey == 0 ? 'frist-openTool' : ''"
    @touchmove.stop.prevent="moveHandle('touchmove')"
    @click="moveHandle('click')"
    v-if="show"
  >
    <view class="openTool-wx" :animation="animationData">
      <view class="openTool-wx-list" v-if="data">
        <view
          class="openTool-wx-list-item"
          @click="copyText(data.content)"
          v-if="data.msgType == 'TEXT'"
        >
        ...
        <view
          ...
          @click="shoucang"
          v-if="
            data.msgType !== 'TRTC_VOICE_END' &&
            data.msgType !== 'TRTC_VIDEO_END'
          "
        >
          <view class="openTool-wx-list-item-icon"
        ...
        <view class="openTool-wx-list-item" @click="chexiao">
          <view class="openTool-wx-list-item-icon"
            ><view class="wxfont wxdelete"></view
          ></view>
          <view class="text">单方撤回</view>
        </view>
      </view>
...
export default {
  ...
  methods: {
    ...
    shanchu() {
      ...
      var chatWindowData =
        this.$store.state.chatDatalist[this.talkTo.userId].list
