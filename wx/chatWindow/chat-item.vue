<template>
  ...
  <cell>
    ...
    <view
      class="zfb-tk-item"
      :class="[
        { 'zfb-tk-msgleft': type == 'left' },
        { 'zfb-tk-msgright': type == 'right' },
        { 'zfb-tk-msgcenter': item.type == 3 }
      ]"
    >
      <openTool
        :class="[
          { 'openTool-msgleft': item.type == 1 },
          { 'openTool-msgright': item.type == 2 }
        ]"
        :talkTo="talkTo"
        :ref="'toolx' + itemKey"
        :data="item"
      ...
      <image
        class="zfb-tk-avatar"
        @click="gochatOne(item)"
        v-if="item.type !== 3"
        :src="
          avatar > 0 ? `../../static/img/avatar/avatar${avatar}.jpg` : avatar
        "
        mode="aspectFill"
      ></image>
      <view
        class="zfb-tk-item-contentx"
        @longpress="longpressItem($event, itemKey, item)"
      >
        <view class="zfb-tk-item-contentx-c">
          <view
            class="zfb-tk-time-notsend wxfont fssb"
            @click="tryagin(item, itemKey)"
            v-if="item.sendtype && item.sendtype == 'error'"
          ></view>
          <view class="zfb-tk-item-contentx-c-tool">
            <view class="zfb-tk-item-c" v-if="item.messageType == '1'">
              <text>{{ item.content }}</text>
            </view>
            <view class="zfb-tk-item-c" v-if="item.messageType == 'ALERT'">
              <text>{{ item.content }}</text>
            </view>
            <view
              class="zfb-tk-item-c-LOCATION"
              v-if="item.messageType == 'LOCATION'"
              @click="goMap(returnParse(item.content))"
            >
              <view class="zfb-tk-item-c-LOCATION-name">{{
                returnParse(item.content).name
              }}</view>
              <view class="zfb-tk-item-c-LOCATION-address">{{
                returnParse(item.content).address
              }}</view>
              <image
                class="zfb-tk-item-c-LOCATION-map"
            ...
            >
              <view class="zfb-tk-item-c-VOICE-msg">
                <view
                  class="zfb-tk-item-c-VOICE-tras"
                  @click="showTrs = !showTrs"
                >
                  <text>转文字</text>
                ...
                <view
                  class="zfb-tk-item-c"
                  @click="playVOICE(returnParse(item.content).url)"
                >
            ...
            <view
              v-if="item.messageType == 'TRTC_VOICE_END'"
              class="zfb-tk-item-c-TRTC_VOICE_END"
              @click="sendVoiceCall"
            >
            ...
            <view
              v-if="item.messageType == 'TRTC_VIDEO_END'"
              class="zfb-tk-item-c-TRTC_VIDEO_END"
              @click="sendVideoCall"
            >
              <view class="zfb-tk-TRTC zfb-tk-item-c">
            ...
            >
              <view class="zfb-tk-item-c-CARD-top">
                <image
                  class="zfb-tk-item-c-CARD-top-img"
                  :src="returnParse(item.content).avatar"
                  mode=""
                ></image>
