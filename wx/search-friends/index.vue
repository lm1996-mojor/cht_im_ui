      ...
      <view class="search-friends-list">
        <view class="search-friends-list-item" v-for="(v, i) in list" :key="i">
          <user-avatar
            ...
            :modelValue="v.senderInfo.avatar"
            @click="godetail(v)"
          ></user-avatar>
          <view class="search-friends-list-user" @click="godetail(v)">
            <view class="search-friends-list-username">{{
              v.senderInfo.nickname
            }}</view>
...
export default {
  ...
  methods: {
    godetail(e) {
      ...
      uni.navigateTo({
        url:
          '../personInfo/addFriendsDetail?applyId=' +
          e.id +
          '&applyerId=' +
          e.senderInfo.id +
          '&status=' +
    ...
    subform(e) {
      ...
      if (checkRes) {
        uni.navigateTo({
          url:
            '../personInfo/addFriendsDetail?param=' +
            this.form.param +
            '&type=1'
        })
      } else {
