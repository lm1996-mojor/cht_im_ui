<template>
	<view class="note-list-page">
		...
		<view class="tab-nav">
			<view 
				class="tab-item" 
				:class="activeTab === item.type ? 'active' : ''"
...
<script>
	export default {
		...
		methods: {
			...
			changeTab(tabType) {
				if (this.activeTab === tabType || this.isLoading) return;
				this.activeTab = tabType;
				this.queryParams.searchType = tabType;
				this.handleRefresh();
			},
			handleRefresh() {
		...
		onNavigationBarButtonTap(e) {
			switch (e.index) {
				case 0:uni.navigateTo({url: '../circles/create'});break;
				default:break;
			}
		},
