<template>
	<view class="note-page-wrap">
		...
		<scroll-view scroll-y class="note-create-page">
			...
			<view class="form-item">
				<zmm-upload-image chooseType="chooseMedia" @oneComplete="upLoadoneComplete" ref="upup"
					v-model="noteForm.avatar" :limit="1"></zmm-upload-image>
			</view>

			<!-- 2. 圈子标题 -->
			<view class="form-item">
				<view class="form-label">圈子标题</view>
				<input v-model="noteForm.name" class="form-input" placeholder="请输入圈子标题" />
			</view>

			<!-- 3. 圈子描述 ✅【核心修复：新增完整工具栏 + 保留取值逻辑】 重点区域 -->
			<view class="form-item">
				<view class="form-label">圈子描述</view>
				<!-- ✅ 新增 富文本工具栏 - 必须加这个标签才会显示工具栏 -->
						...
						<view class='wrapper'>
							...
							<view class="editor-wrapper">
								<editor id="editor" class="ql-container" placeholder="开始输入..." show-img-size
									...
									:read-only="readOnly" @ready="onEditorReady" @input="handleEditorInput">
								</editor>
							</view>
						</view>
					</view>
			...
			<view class="form-item">
				...
				<radio-group class="radio-group" @change="radioChange">
					<label class="radio-item">
						<radio :value="1" :checked="noteForm.joinType === 1" /> 仅邀请
					</label>
					<label class="radio-item">
						<radio :value="2" :checked="noteForm.joinType === 2" /> 申请审核
					</label>
				</radio-group>
			</view>
		...
		<!-- 底部固定按钮区 -->
		<view class="btn-box">
			<button type="default" class="btn" @click="resetForm">重置表单</button>
			<button type="primary" class="btn submit-btn" @click="submitNote">提交圈子</button>
		</view>
	</view>
</template>
	...
	export default {
		...
		data() {
			return {
				readOnly: false,
				formats: {},
				// ✅ 严格匹配Request接口的4个核心字段 无任何冗余
				noteForm: {
					name: "", // 圈名称
					avatar: [], // 头像【数组临时存储】→ 提交时转字符串
					description: "", // 圈简介 【已修复取值为空问题】
					joinType: 1 // 加入方式 默认值：1=仅邀请
				},
				editorCtx: null, // 富文本编辑器实例
				// ✅ 新增：工具栏配置项 - 配置需要显示的功能按钮（常用功能都配好了，可自定义增删）
				toolbarOptions: {
					toolbars: [
						['bold', 'italic', 'underline', 'strike'], // 加粗、斜体、下划线、删除线
						['alignLeft', 'alignCenter', 'alignRight'], // 左对齐、居中、右对齐
						['insertDivider', 'clear'], // 分隔线、清空内容
					]
				},
				// ✅ 修复上传方法的变量报错问题（之前未定义）
				limit: 9,
		...
		methods: {
			...
			// ✅ 富文本初始化 - 关联编辑器+工具栏，初始化完成后正常显示
			// onEditorReady() {
			// 	uni.createSelectorQuery().in(this).select('.rich-editor').context(res => {
			// 		this.editorCtx = res.context;
			// 	}).exec();
			// },
			// ✅ 保留上一轮核心修复：实时获取富文本内容，赋值给description（必填，不然拿不到值）
			handleEditorInput(e) {
				// 优先取带标签的富文本内容，保留排版格式，适配接口string类型
				this.noteForm.description = e.detail.html;
				// 如需纯文本无标签，切换这行：this.noteForm.description = e.detail.text;
			},
			// 加入方式单选切换 - 逻辑不变
			radioChange(e) {
				const val = e.detail.value;
				this.noteForm.joinType = val ? Number(val) : null;
			},
			// 重置表单 - 重置4个核心字段+富文本内容
			resetForm() {
				this.noteForm = {
			...
			onEditorReady() {
				...
				uni.createSelectorQuery().select('#editor').context((res) => {
					...
					this.editorCtx.setContents({
						// html: "11<b>111<u>1<i>1</i>1</u>11</b>111", // 默认内容
					})
				}).exec()
				// #endif
			},
			...
			submitNote() {
				...
				const submitData = {
					...
					joinType: this.noteForm.joinType,
					avatar: this.noteForm.avatar.length > 0 ? this.noteForm.avatar[0].url : null
				};
				console.log('提交参数(含富文本内容)：', submitData);
				this.$http.request({
