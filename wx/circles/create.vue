	...
	<view class="note-page-wrap">
		...
		<scroll-view scroll-y class="note-create-page">
			...
			<view class="form-item flex-row">
				...
				<view class="select-box" @click="openJoinTypeDrawer">
					<text class="select-text">{{ joinTypeText }}</text>
		...
		<!-- 底部固定按钮区 -->
		<view class="btn-box">
			<button type="primary" class="btn submit-btn" @click="submitNote">创建</button>
		</view>
		...
		>
			<view class="drawer-content">
				<view class="drawer-header">
					...
					<text class="iconfont icon-close" @click="closeJoinTypeDrawer"></text>
				</view>
				<view class="drawer-body">
					<view 
						class="drawer-item" 
						:class="{ active: noteForm.joinType === 1 }"
						@click="selectJoinType(1)"
					>
						<text class="item-text">仅邀请</text>
						<text class="iconfont icon-check" v-if="noteForm.joinType === 1"></text>
					</view>
					<view 
						class="drawer-item" 
						:class="{ active: noteForm.joinType === 2 }"
						@click="selectJoinType(2)"
					>
						<text class="item-text">申请审核</text>
						<text class="iconfont icon-check" v-if="noteForm.joinType === 2"></text>
					</view>
				</view>
			</view>
		</uni-drawer>
...
<script>
	export default {
		...
		computed: {
			// 加入方式文本显示
			joinTypeText() {
				return this.noteForm.joinType === 1 ? '仅邀请' : '申请审核';
			}
		},
		methods: {
			// 头像上传回调
			...
			// 打开加入方式选择抽屉
			openJoinTypeDrawer() {
				this.$refs.joinTypeDrawer.open();
			},
			
			// 关闭加入方式选择抽屉
			closeJoinTypeDrawer() {
				this.$refs.joinTypeDrawer.close();
			},
			
			// 选择加入方式
			selectJoinType(type) {
				this.noteForm.joinType = type;
				this.closeJoinTypeDrawer();
			},
