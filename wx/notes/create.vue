<template>
	<view class="note-page-wrap">
		...
		<!-- 引入预览组件 -->
		<note-preview v-if="previewShow" :content-list="contentList" :visible="previewShow"
			@close="previewShow = false" />
		<!-- 唯一滚动区域 -->
		<scroll-view scroll-y class="note-create-page">
			<!-- 内容块列表 - 支持拖拽排序 -->
			<view class="content-block" v-for="(item, index) in contentList" :key="item.id"
				:style="{marginBottom: '30rpx'}">
				<!-- 排序拖拽按钮 -->
				<view class="drag-handle" @touchstart="(e) => handleDragStart(e, index)"
					@touchmove="(e) => handleDragMove(e, index)">
					<text class="drag-icon">☰</text>
				</view>

				<!-- 标题块 - 新增删除按钮 -->
				<view v-if="item.dataType === 'title'" class="block-content title-block">
					<input v-model="item.content" class="title-input" placeholder="请输入标题" />
					<view class="delete-btn" @click="deleteBlock(index)">删除</view>
				</view>

				<!-- 文本块 - 新增删除按钮 -->
				<view v-if="item.dataType === 'text'" class="block-content text-block">
					<textarea v-model="item.content" class="text-input" placeholder="请输入文本内容" auto-height />
					<view class="delete-btn" @click="deleteBlock(index)">删除</view>
				</view>
		...
		<view class="operate-bar">
			...
			<view class="btn">
				<button style="flex:1" class="preview-btn" @click="showPreview">预览</button>
				<button style="flex:1" class="submit-btn" @click="submitNote">提交</button>
			</view>
		</view>
		<uni-drawer ref="circleDrawer" mode="right" :mask-click="true" :show="false" :width="windowWidth"
			class="circle-drawer">
			<view class="drawer-content">
				...
				<scroll-view scroll-y style="flex: 1;">
					<friend-list ref="circleListRef" :default-selected-id="noteForm.circleId"
						@select="handleCircleSelect"></friend-list>
				</scroll-view>
				<!-- ✅ 确认按钮 现在正常显示了 -->
...
<script>
	export default {
		...
		methods: {
			...
			// 更新所有内容块的位置信息（拖拽用）
			updateBlockRects() {
				uni.createSelectorQuery().in(this).selectAll('.content-block').boundingClientRect(rects => {
			...
			handleDragMove(e) {
				...
				if (overIndex !== -1 && overIndex !== this.dragStartIndex && this.dragOverIndex !== overIndex) {
